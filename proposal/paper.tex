...
{\em Mar 29 to Apr 12}
See page~[\pageref{Mar20toApr12}] of section~\ref{sec:UseOfWikipedia}
\begin{itemize}
...
\begin {enumerate}
\item For each of them we use a basic exponential algorithm that takes all possible combinations of the content words.
\item The default configuration of T-verifier generates a total of 5 $AU$s, without any ranking.  For each content wordin a $AU$, or a combination of words in case the $AU$ consists of multiple content words, we check if there exists a wikipedia article. There may be three cases:
\begin{enumerate}
...
\end{enumerate}
\item Similarly for $DU$ we check all possible combinations.
\end{enumerate}
\item We preporcess the wikiaticles by stripping them of html, and converting them into simple bag of words i.e we retain word repetation.
...
\item Use the heading, one sentence summary in the disambiguation page adjacent to the page name, and categories tagged to that article page for each of the disambiguation item.
\end{enumerate}
For this report, we manually did the disambiguation and found the results were comparitively better.

\paragraph{Word matching}
We found that the intersection of the two bags of words, $W_{au_{i}}$ and $W_{tu_{i}}$ produced different results. This is because the regular expression matcher we are using, grep finds the maximal match. Lets consider that the first file, $W_{au_{i}}$ has "invent", and the second file, $W_{tu_{i}}$ has "invented", and "inventor". So when we pick a word in $W_{au_{i}}$ and try to match words in $W_{tu_{i}}$, we will find 2 matches. However, the words from second file, both "invented", and "inventor" will not find any matches in ($W_{au_{i}}$). Taking both way intersections will ensure we do not miss out different morphological forms of the same word appearing in the context common to the both articles.
...
\item Wiki articles detected from TU($W_{tu_{i}}$): The\_guitar, The\_Electric\_guitar  Electric\_guitar, invented \rightarrow Invention 
\end{enumerate}
\end{enumerate} 
